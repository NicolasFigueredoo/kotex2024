    ...
    <div class="container">
        ...
        <div class="w-100 border-bottom d-flex justify-content-between">
            ...
            <button @click="crearValores()" type="button" class="btn mb-1"
                 id="crearServicio"   style="background-color: rgb(52, 68, 127); color: white;">Crear valores</button>
        </div>
            ...
            <tbody>
                <tr v-for="seccion in secciones" :key="seccion.id">
                    ...
                    <td>
                        ...
                        <button type="button" class="btn btn-sm btn-danger" style="margin-left: 15px; " @click="deleteValor(seccion.id)">
...
methods:{
    ...
    deleteValor(idValor){
        axios.post('/api/deleteValor',{
            idValor: idValor
        })
                .then(response => {
                    this.$store.commit('setMostrarAlerta', true);
                    this.$store.commit('setClaseAlerta', 1);
                    this.$store.commit('setMensajeAlerta', 'Valor eliminado con éxito');  
